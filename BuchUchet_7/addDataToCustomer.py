...
from addDataCustomer import Data
from checkFormatData import Format



class AddDataCustomer():

    def __init__(self,slave,nameFirm):
        self.slave = slave
        self.nameFirm = nameFirm
        self.data = Data(self.slave,'customer',self.nameFirm)
        self.balans = '----'
        self.db = Base('customer')
        self.format = Format()

    def addForm(self):
        Grid.rowconfigure(self.slave, 0, weight=1)
        Grid.columnconfigure(self.slave, 0, weight=1)
        self.cus = Toplevel(self.slave,bg='#20BAB5')
        self.cus.title('Данные по %s' % (self.nameFirm))
        self.cus.geometry('1000x650+115+100')
        self.cus.grab_set()
    ...
    def frameDialog(self):
        Grid.rowconfigure(self.cus, 1, weight=1)
        Grid.columnconfigure(self.cus, 0, weight=1)
        frame2 = Frame(self.cus,width=1000,height=95,bg='#20BAB5')
        ...
        # Button
        buttonAdd = Button(frame2,font="Arial 12 bold",width=15,height=1,text="Завести данные",bg='#E1E9E8')
        buttonAdd.place(x = 11, y = 55)
        buttonAdd.bind('<Button-1>', lambda _: self.data.formAddData())
        buttonEdit = Button(frame2,font="Arial 12 bold",width=15,height=1,text="Редактировать",bg='#E1E9E8',command=self.editCustomerData).place(x = 180, y = 55)
        buttonDel = Button(frame2,font="Arial 12 bold",width=12,height=1,text="Удалить",bg='#E1E9E8',command=self.deleteCustomerData).place(x = 350, y = 55)
        buttonPrint = Button(frame2,font="Arial 12 bold",width=16,height=1,text="Сохранить в файл",bg='#E1E9E8',command=self.printToFile).place(x = 490, y = 55)
        buttonExit = Button(frame2,font="Arial 12 bold",width=12,height=1,text="Выход",bg='#E1E9E8',command=self.quit).place(x = 670, y = 55)
        buttonReload = Button(frame2,font="Arial 12 bold",width=12,height=1,text="Обновить",bg='#E1E9E8',command=self.replace).place(x = 810, y = 55)

    def frameBox(self):
        self.lists = []
        Grid.rowconfigure(self.cus, 2, weight=100)
        Grid.columnconfigure(self.cus, 0, weight=100)
        frame1 = Frame(self.cus,width=1000,height=450)
        frame1.grid(row=2,column=0, rowspan=1, columnspan=1, sticky=N + S + E + W)
        Label(frame1, text=self.nameFirm,font="Arial 13 bold",bd=15,bg='#20BAB5').pack(side=TOP, fill=X)


        frameb1 = Frame(frame1); frameb1.pack(side=LEFT, expand=YES, fill=BOTH)
        Label(frameb1, text='Дата\nоперации',borderwidth=1, relief=RAISED, font="Arial 12 bold",bg='#20BAB5').pack(fill=X)
        ...
        Label(frameb2, text='Отгрузили на\nсумму (в рублях)', borderwidth=1, relief=RAISED, font="Arial 12 bold",bg='#20BAB5').pack(fill=X)
        ...
        Label(frameb3, text='Пришло денежных\n средств (в рублях)', borderwidth=1, relief=RAISED, font="Arial 12 bold",bg='#20BAB5').pack(fill=X)
        ...
        for datePaym, otgruz, oplata, comment in self.resault:
            #if str(datePaym.encode('utf-8')) == '' or str(datePaym.encode('utf-8')) == '-':
            if str(datePaym) == '' or str(datePaym) == '-':
                break
            else:
                lb1.insert(END, ' %s' % datePaym)
                lb2.insert(END, '%s' % str(otgruz))
                lb3.insert(END, '%s' % str(oplata))
                comment = comment.encode('utf-8')
                lb4.insert(END, '%s' % str(comment))

    def frameBal(self):
        Grid.rowconfigure(self.cus, 3, weight=1)
        Grid.columnconfigure(self.cus, 0, weight=1)
        frameB = Frame(self.cus,width=1000,height=105,bg='#20BAB5')
        frameB.grid(row=3,column=0, rowspan=1, columnspan=1, sticky=N + S + E + W)
        ############## 1 способ
        #Label(frameB, text="Остаток на начало периода:",font="Arial 12 bold",bg = '#20BAB5').place(x = 10, y = 15)
        #Label(frameB, text=str(self.balansStart),font="Arial 12 bold",bg = '#20BAB5').place(x = 300, y = 15)
        #Label(frameB, text="Оборот за выбранный период:",font="Arial 12 bold",bg = '#20BAB5').place(x = 10, y = 40)
        #Label(frameB, text=str(self.balansSum),font="Arial 12 bold",bg = '#20BAB5').place(x = 300, y = 40)
        #Label(frameB, text="Остаток на окончание периода:",font="Arial 12 bold",bg = '#20BAB5').place(x = 10, y = 65)
        #Label(frameB, text=str(self.balansEnd),font="Arial 12 bold",bg = '#20BAB5').place(x = 300, y = 65)

        ############## 2 способ
        Label(frameB, text="",font="Arial 3 bold",bg = '#20BAB5').grid(row=0,column=0,padx=15,sticky=W)
        Label(frameB, text="Остаток на начало периода:   ",font="Arial 12 bold",bg = '#20BAB5').grid(row=1,column=0,padx=15,sticky=W)
        #Label(frameB, text="Оборот за выбранный период:   ",font="Arial 12 bold",bg = '#20BAB5').grid(row=2,column=0,padx=15,sticky=W)
        Label(frameB, text="Остаток на окончание периода:   ",font="Arial 12 bold",bg = '#20BAB5').grid(row=2,column=0,padx=15,sticky=W)
        Label(frameB,text=str(self.balansStart),font="Arial 12 bold",bg = '#20BAB5').grid(row=1,column=1,sticky=E)
        #Label(frameB,text=str(self.balansSum),font="Arial 12 bold",bg = '#20BAB5').grid(row=2,column=1,sticky=E)
        Label(frameB,text=str(self.balansEnd),font="Arial 12 bold",bg = '#20BAB5').grid(row=2,column=1,sticky=E)
        Label(frameB, text="Отгрузили за период на сумму:  ",font="Arial 12 bold",bg = '#20BAB5').grid(row=1,column=2,padx=50,sticky=W)
        Label(frameB, text="Оплатили нам за период на сумму:  ",font="Arial 12 bold",bg = '#20BAB5').grid(row=2,column=2,padx=50,sticky=W)
        Label(frameB,text=str(self.payCurr),font="Arial 12 bold",bg = '#20BAB5').grid(row=1,column=3,sticky=E)
        Label(frameB,text=str(self.depayCurr),font="Arial 12 bold",bg = '#20BAB5').grid(row=2,column=3,sticky=E)
        Label(frameB, text="",font="Arial 12 bold",bg = '#20BAB5').grid(row=1,column=4,padx=40,sticky=W)
    ...
    def serv(self,slave):
        ...
        self.menu.add_command(label="Редактировать", command=self.editCustomerData)
        self.menu.add_command(label="Удалить", command=self.deleteCustomerData)
    ...
    def printToFile(self):
        ...
        try:
            ...
            with open(self.pathFile, 'w') as file:
                ...
                file.write('Отгрузили за период на сумму : %s%s' % (self.payCurr,'\n'))
                file.write('Оплатили нам за период на сумму : %s%s' % (self.depayCurr,'\n'))
            file.close()
            self.ok()
    ...
    def ok(self):
        self.saveWind = Toplevel(self.cus)
        self.saveWind.title('Выгрузка данных в файл')
